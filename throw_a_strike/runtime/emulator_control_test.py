...
from throw_a_strike.rendering import render_style_selection_rgb888, render_throw_control_rgb888
...
class EmulatorControlTestStep:
    ...
    def __post_init__(self):
        ...
        if type(self.framebuffer) is not bytes or len(self.framebuffer)!=49152: raise InvalidPortValueError("framebuffer must be exact RGB888 bytes")
        if type(self.framebuffer_accepted) is not bool: raise InvalidPortValueError("framebuffer_accepted must be exact bool")
        if (self.phase is EmulatorControlTestPhase.SELECT_STYLE) != (self.presentation is None): raise InvalidPortValueError("phase and presentation are inconsistent")
        if self.phase is not EmulatorControlTestPhase.SELECT_STYLE and not self.selection.confirmed: raise InvalidPortValueError("attempt requires confirmed selection")
        if self.phase is EmulatorControlTestPhase.RECOVERY_HOLD and self.presentation.phase is not ThrowControlPhase.EARLY_DART_RECOVERY: raise InvalidPortValueError("recovery phase inconsistent")
        if self.phase is EmulatorControlTestPhase.TERMINAL and not self.presentation.terminal: raise InvalidPortValueError("terminal phase inconsistent")
    @property
