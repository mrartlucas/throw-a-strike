...
class PinfallAnimationTests(unittest.TestCase):
    ...
    def footprint(self, frame, cx, cy):
        return tuple(self.pixel(frame, x, y) for y in range(cy - 3, cy + 4) for x in range(cx - 3, cx + 4))
    ...
    def test_future_active_finished_waves_and_exact_timing(self):
        ...
        active = render_pinfall_rgb888(self.presentation, self.setup, PlayerColor.BLUE, self.sample, self.resolution, 0.121)
    ...
    def test_local_progress_zero_matches_upright_then_changes_and_disappears(self):
        upright = render_round_throw_rgb888(self.presentation, 1, 1, PlayerColor.BLUE, standing_pins=(2,))
        at_start = render_pinfall_rgb888(self.presentation, self.setup, PlayerColor.BLUE, self.sample, self.resolution, 0.120)
        advanced = render_pinfall_rgb888(self.presentation, self.setup, PlayerColor.BLUE, self.sample, self.resolution, 0.121)
        gone = render_pinfall_rgb888(self.presentation, self.setup, PlayerColor.BLUE, self.sample, self.resolution, 0.420)
        self.assertEqual(self.footprint(at_start, 54, 56), self.footprint(upright, 54, 56))
        self.assertNotEqual(self.footprint(advanced, 54, 56), self.footprint(upright, 54, 56))
        self.assertEqual(self.pixel(gone, 54, 56), LANE)
        before_next = render_pinfall_rgb888(self.presentation, self.setup, PlayerColor.BLUE, self.sample, self.resolution, 0.239)
        upright_five = render_round_throw_rgb888(self.presentation, 1, 1, PlayerColor.BLUE, standing_pins=(5,))
        self.assertEqual(self.footprint(before_next, 64, 40), self.footprint(upright_five, 64, 40))

    def test_miss_and_gutter_result_labels_are_rendered(self):
        labels = []
        original = deck_renderer._text
        for setup, expected in ((ThrowSetup(ControlStyle.QUICK, 0, 64, 84, CurveLevel.STRAIGHT, 70), "MISS"),
                                (ThrowSetup(ControlStyle.QUICK, 0, 0, 4, CurveLevel.STRAIGHT, 70), "GUTTER")):
            trajectory = build_ball_trajectory(setup)
            resolution = resolve_ball_pinfall(trajectory)
            sample = sample_ball_roll(trajectory, resolution, trajectory.duration_seconds)
            with patch.object(deck_renderer, "_text", side_effect=lambda b, t, x, y, c, scale=1: (labels.append(t), original(b, t, x, y, c, scale))[1]):
                render_throw_result_rgb888(self.presentation, setup, PlayerColor.BLUE, sample, resolution)
            self.assertIn(expected, labels)

    def test_recovery_and_foul_frames_contain_no_ball(self):
        recovery_machine = ThrowControlMachine(ControlStyle.ADVANCED, 0)
        recovery_machine.apply(ThrowControlCommand(ThrowControlCommandKind.DART_HIT, 1, 0, 64, 84))
        recovery = build_throw_control_presentation(recovery_machine.snapshot)
        foul_machine = ThrowControlMachine(ControlStyle.QUICK, 0)
        foul_machine.apply(ThrowControlCommand(ThrowControlCommandKind.TICK, 31))
        foul = build_throw_control_presentation(foul_machine.snapshot)
        for frame in (render_round_throw_rgb888(recovery, 1, 1, PlayerColor.BLUE),
                      render_round_throw_rgb888(foul, 1, 1, PlayerColor.BLUE)):
