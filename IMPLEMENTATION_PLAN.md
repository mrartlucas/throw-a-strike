...
- **Timing and terminology:** **THROW READY** begins its own walkback/throw timer. Warning activates at 30 seconds; at 60 seconds the terminal result is **FOUL**, with no setup. Curve and power selection each default deterministically after eight seconds; a sparse command traverses every reached deadline.
...
- **Timing:** a clock tick at 30 seconds activates warning and one at the exact 60-second THROW READY deadline creates FOUL with no `ThrowSetup`. `started_at`, event timestamps, and clock values must share the same monotonic domain; values are never offset, clamped, repaired, sorted, or reordered.
